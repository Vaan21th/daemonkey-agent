...
重启后自动触发 background LLM turn ( III 补丁 3 · 2026-05-26)

----------------------------------------------------------------------
为什么有这个模块
----------------------------------------------------------------------

用户 反馈: 当前 daemon 重启续场只在 session jsonl 里注一条 system message ·
但 LLM 不会被自动唤醒 · 用户 必须发一条消息触发 chat handler · Daemonkey 才能
读到 system message · 才能继续上次的任务。

用户 期望: Daemonkey 调 request_restart 时可以塞一个 follow_up_message ·
重启后新 daemon 自动以这条作为 user message 触发 background LLM turn ·
Daemonkey 跑完结果落档进 session jsonl · 用户 下次进 WebUI 就看到验证结论。

----------------------------------------------------------------------
设计取舍
----------------------------------------------------------------------

1. **后台 turn 没有前端 SSE 接收方**: progress=None · 跑完落档 · 用户 进 WebUI 翻历史看
2. **auto_confirm = 'confirm'** ( 续 14 补丁 IV · 2026-05-26 · 用户 拍板):
   - 跟主对话 WebUI 默认 'confirm' **同级对齐** · 不再比主对话严一档
   - AUTO + CONFIRM **自动 go** (curl / shell_exec / python_exec / git commit /
     write_file 等都能跑 · Daemonkey 真有能力验证 endpoint / 跑 HTTP / 落档代码)
   - GUARD (rm / empty_trash / git push --force / 大改文件) 走 inline confirm ·
     **背景 turn 没 SSE 接收方** → 自动 skip = Daemonkey 看到 declined → Daemonkey 在 turn 里
     把要 GUARD 的事讲给 用户 · 用户 切回 WebUI 手动处理
...
# 续 14 补丁 IV · 跟主对话 WebUI 默认 'confirm' 对齐 · 不再严一档
...
#
# · 2026-06-03 · 加 "scheduled" 态根治"重启后假死/输入没锁":
...
def _run_background_turn(message: str, session_id: str) -> dict:
    ...
    返 dict (跟 _chat_impl 一样) 或 raise · 调用方负责包 try/except 续 14 补丁 VI · 2026-05-26 16:15:
      ...
      (不然 用户 reload session 看到 follow_up turn 跑一半的快照 · 后续 reply 不出现 ·
...
def schedule_resume_turn(restart_req: Optional[dict]) -> bool:
    ...
    # · 防呆 (2026-07-28): DeepSeek 调 request_restart 时常不传 follow_up_message
    # (null/空串) · 用户 只能手动戳。 从 session 历史自动摘最后一条 user 消息续场。
        ...
        try:
            ...
            for m in reversed(msgs):
                if m.get("role") == "user" and m.get("content") and m["content"].strip():
                    ...
                    follow_up = f"(自动续场) 用户 上次说: {last_user[:120]}"
                    break
        except Exception:
            pass
    if not follow_up:
    ...
    # · 关键: 一确定要续写就同步置 "scheduled" (在 spawn 线程 + _wait_runtime_ready 之前)。
    ...
    def _runner():
        # IV (2026-05-26 第二十二根毛): flush=True · 子进程 stdout
        # redirect 到文件时默认 block-buffered (4096 bytes) · 不 flush 用户 看不到 log
