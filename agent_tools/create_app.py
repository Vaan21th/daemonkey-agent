...
SPEC = ToolSpec(
    ...
    description=(
        ...
        "  审 N 段片段 / 对比 N 个方案) · 把 `dispatch_subagent` 放进 tools · app 内部就能\n"
        "  一次把这些子任务并行派给隔离的分身、跑完自动汇总回来 —— 比 app 自己串着一个个做\n"
        "  省时 · 且每个分身独立上下文不互相污染。 system_prompt 的『## 动作』里写清: 先拆出\n"
        "  子任务清单 · 再一次性调 dispatch_subagent (tasks 数组·一子任务一项) · 最后汇总。\n"
