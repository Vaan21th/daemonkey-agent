...
from desktop_pet.room_rest import (
    after_getup,
    arm_rest,
    go_rest,
    take_cmd,
...
def finish_once(pet) -> None:
    ...
    if pet._getting_up:
        ...
        return
    pet._poking = False
    # work_done 是收工片 · 哪怕 state 还停在 working 也回待机, 别把干活循环接回去
...
def _poll_while_down(pet) -> None:
    """趴着 / 起身时：完成通知和情绪片不许打断，只有对话/干活能叫醒。"""
    write_live(pet)
    try:
    ...
    try:
        ...
        if txt in WORK_STATES and txt != pet._last_state_txt:
