...
卷四十四 K stage 2c · 出品工坊资产层 · apps + workflows
卷四十四 K stage 2c++ · wish-6fd76512 · 软删 + 回收站 (2026-05-25 第十六根毛)
卷四十六续 12 · wish-165ea1f6 phase A · ui_form_schema (2026-05-26 第二十一根毛)
卷四十六续 12 · wish-165ea1f6 phase B · output_schema (2026-05-26 第二十一根毛)
卷四十六续 13 · wish-165ea1f6 phase C · exec_kind + exec_template (2026-05-26 第二十一根毛)
...
workshop_assets.py (本文件)
    新形态: app + workflow 资产 · 一个个 json 文件 · 给 OPUS 自演进用
    - app: 独立模块 · 一个被 OPUS prompt + 工具白名单封装的子能力
...
exec_kind / exec_template 字段说明 (wish-165ea1f6 phase C · 2026-05-26 主对话 OPUS 提出):
...
def save_app(spec: dict) -> dict:
    ...
    payload = {
        ...
        "created_by": (spec.get("created_by") or "OPUS").strip(),
...
def save_flow(spec: dict) -> dict:
    """落一个 workflow · spec 必须含 name + description + litegraph_json"""
    spec = dict(spec or {})
    name = (spec.get("name") or "").strip()
    description = (spec.get("description") or "").strip()
    ...
    if graph is None or not isinstance(graph, dict):
        raise ValueError("litegraph_json must be a dict (LiteGraph.serialize() output)")
...
def _atomic_write(path: Path, content: str) -> None:
    """原子写——卷四十六 III · wish-badd4 收编到 safe_write
    workshop assets registry (app / workflow / templates) 是 BRO 自建作品·backup=True"""
...
def _sanitize_app(data: dict) -> dict:
    ...
    return {
        ...
        "created_by": data.get("created_by") or "OPUS",
