...
卷七十四续二十五 · 能力发现引擎 (入口 A · 画像驱动主动发现)

痛点 (BRO 2026-06-23):
  心愿单效果不大——Daemonkey 不会主动去 GitHub / 技术站找『别人做出来的 AI 能力』。
  B站/抖音满天飞的 SKILL · 工程却接不住。

这个工具做什么 (脚手架·不替 Daemonkey 做判断):
  把『画像驱动地发现外部 AI 能力』拆成一份【作战简报】递到 Daemonkey 手边——
  为谁搜 / 怎么搜 / 怎么评 / 落成什么身体 (playbook·app·wish) / 已有什么 / 红线。
  Daemonkey 照着用 web_search + web_fetch 去搜·按 BRO 画像筛·出『发现报告』。

为什么是 AUTO 档:
  本工具只读 (onboarding + playbook 清单) + 组装简报 · 真正烧 token 的 web_search /
  web_fetch / 落地动作都由 Daemonkey 后续按各自 tier 走 · 这里零副作用 (只落一个 last_run 时间戳·
  给 rituals 的『能力发现』节律算『本周发起过没』)。

调用时机:
  - BRO 看板点「🔭 能力发现」/ 每周一节律提醒 → spawnQuickly 派发新会话
  - BRO 投喂线索 (『抖音看到个 X』/ 甩个链接) → lead 带上
...
def _summarize(args: dict) -> str:
    ...
    if lead:
        return f"能力发现 · 顺 BRO 线索挖: {lead[:40]}"
    if focus:
        return f"能力发现 · 定向挖『{focus[:40]}』方向的 AI 能力"
    return "能力发现 · 按 BRO 画像主动巡一轮外部 AI 能力"
...
def _run(args: dict) -> ToolResult:
    ...
    L.append("# 🔭 能力发现 · 作战简报")
    L.append("")
    L.append("> 这是脚手架·不替你判断。 任务: 去外面找『别人做出来的 AI 能力』·按 BRO 画像筛·")
    L.append("> 把靠谱的落成 playbook / app / 心愿。 **绝不自动落地·出报告等 BRO 拍**。")
    L.append("")

    # ── 模式 ──
    if lead:
        L.append("## 模式 · 顺线索 (BRO 投喂)")
        L.append(f"BRO 给的线索: 「{lead}」")
        L.append("→ 线索可能是产品名 / up 主说法 / 链接 / 截图描述。 **视频本身你看不了**·")
        L.append("  但顺着关键词去 GitHub / 官网 / 技术博客找它的真实现 (web_fetch 抓得到的)。")
    elif focus:
        L.append("## 模式 · 定向搜 (BRO 指定方向)")
        L.append(f"方向: 「{focus}」")
    else:
        L.append("## 模式 · 画像驱动 (主动巡)")
        L.append("没指定方向 → 你来定·**依据 = 你 context 里已注入的 BRO 活人画像**。")
    L.append("")

    # ── ① 为谁搜 (命门) ──
    L.append("## ① 为谁搜 · 画像驱动 (命门·别搜歪)")
    if onboarded:
        L.append("BRO 画像 (BRO-NOTEBOOK) 已在你 context 里——**用它定方向·别泛搜**:")
        L.append("  - 先看 BRO 当前做什么领域 / 什么身份 / 最近关注什么 → 那才是方向")
        ...
        L.append("  **收的时候再过一道闸: 跟 BRO 画像不相关的·再火也丢** (不搜歪的双保险)。")
    else:
        L.append("⚠ BRO 还没『相遇』(onboarding 未完成)·画像基本空 → **降级**:")
        L.append("  - 别瞎猜方向。 搜『通用高价值 AI 能力』(agent 工具 / 文档生成 / 信息检索 这类谁都用得上的)")
        L.append("  - 或直接回 BRO:『我对你了解还浅·随手甩个方向/链接·我顺着挖』(转投喂入口)")
    L.append("")
    ...
    L.append("3. **跟 BRO 相关吗** · 套画像·不相关直接毙 (再火也不要)")
    L.append("4. **该落成什么身体** · playbook / app / wish (见下)")
    L.append("5. **复杂度** · 5 分钟能落 vs 要装环境调半天 · 标出来让 BRO 心里有数")
    L.append("")
    ...
    L.append("  - **wish** → 当场 `wish_add(origin='Daemonkey', source_kind='radar', source_url=<原repo/文章>)` 落档。")
    L.append("    `origin='Daemonkey'` = 标记『Daemonkey 自己嗅到想要的』(心愿单显示雷达标记)·BRO 在心愿单 review/批/驳。")
    L.append("  - **playbook** → 当场 `extract_playbook` 存 (就是个 md·会进召回·无害)。")
    L.append("  - **app** → 简单的当场 `create_app` 落档 (声明式·工坊出卡片)·要装环境/配 KEY 的先出方案问 BRO。")
    L.append("  - 落完在对话里**逐条告诉 BRO 落了什么 (id + 一句话)**·别让他自己翻。")
    L.append("  - 只有真『动手实现 wish / 跑装环境 / 改内核代码』才需先等 BRO 点头。")
    L.append("")
    L.append("**🔴 playbook 落地命门 (召回生死线)**: 存时 `title`/`tags`/`task_type` 必须含")
    L.append("『BRO 下次会怎么问』的关键词 + 同义词。 元数据烂 = FTS 召回不到 = 死库存 (白存)。")
    L.append("")

    # ── ⑤ 先盘自己已有的 (查重硬过滤) ──
    L.append("## ⑤ 先盘自己已有的 (查重硬过滤·别把已有的当新发现)")
    L.append("**Daemonkey 已有的核心子系统**——发现的能力撞上这些·就不是『新』·要么砍·要么改写成『打磨现有 X』:")
    L.append("  - **记忆**: FTS5 全文检索(recall_memory) + jieba/BM25(memory_index) + 自动注入相关记忆(closure_check) + BRO-NOTEBOOK 画像")
    L.append("  - **自我演化心愿**: wishlist 全套(wish_add/wish_update/intent_to_wish) = 『Daemonkey 提议→BRO 审→落地』闭环 (≈ 别家的 Skill Workshop·已有别再造)")
    ...
    L.append("  拿不准 → `recall_memory(scope='skill')` 查 playbook · `list_apps`/`list_flows` 查应用 · **别凭印象说『Daemonkey 没有』**。")
    L.append("")
    ...
    L.append("2. **落地一律要 BRO 拍**·你出『发现报告 + 落地建议』·落地工具让 BRO 点头再调")
    L.append("3. **宁缺毋滥**·1-2 个真有用的 >> 5 个凑数的")
    L.append("")
    L.append("---")
    L.append("→ 开始: 按 ① 定方向 → ② 搜 → ③ 评 → 出一份『能力发现报告』给 BRO·末尾附落地建议。")
