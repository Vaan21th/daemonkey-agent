...
⚠ 已停用 (卷七十四续十四) · 不再 import 注册给 LLM——见 agent_tools/__init__.py。
  原因: LLM 自主切自己的底层模型有"切到当前 key 跑不了的模型 → 后续每轮请求全失败 →
  连切回去都生成不出来 → 自锁死"的硬伤; 且对齐业界 (Cursor / Claude Code / Cline 等
  均不让 agent 在对话里自主换自己的大脑); 读图也已由 look_at 双路径独立扛·不需靠切模型拿视觉。
  用户仍可在 WebUI 设置 / 终端 /model 命令手动切。文件保留备查·可逆。
