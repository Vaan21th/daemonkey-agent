...
def runtime_context_addendum(daemon_root: Path) -> str:
    ...
    base = (
        ...
        "## wechat_send\n\n"
        "**主动**给用户发微信（文本 / 图片 / 文件）。走官方 iLink 渠道——前提是用户已经在 "
        "设置 → 微信 & 主动 里扫码连上·且 24h 窗口开着（用户最近 24h 在微信跟你说过话）。\n"
        "没连 / 窗口关了·工具会明说·不会假装发出去。\n"
        "时机：长任务终于完成了 / 用户让你 'X 小时后提醒我' / 你做了一件他应该立即知道的事。\n"
        "**不要**：每条 reply 都额外推一份微信（微信来的对话已经自动回了）；\n"
        "         午夜没事干给他发\"在吗\"——他要休息。\n"
        "CONFIRM 档——主动打扰是有形动作，让他点头一次。\n\n"
