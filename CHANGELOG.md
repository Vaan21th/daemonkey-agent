...
**内核大更 · 内测前基线（1.0.0 留给内测之后）**

### 新增 Added
- **工具目录** —— 闲聊不再每轮挂整本工具菜单；核心手全文，其余走 `catalog_search` / `catalog_call`
- **画像核心层** —— 只留改说话/决策的短条，细节走 `recall_memory`
- **折前落盘 + 轨迹** —— 压缩前先把对话里没写盘的短事实落本子；回合轨迹可回放
- **自传骨/肉** —— 文件不动；前缀只灌身份骨，年谱/工程坐实按需召回

### 修复 / 优化 Fixed
- **出厂空槽停灌** —— 空自传、只有「第 1 次（示范）」的日记不再进稳定前缀
- **点停真停** —— 杀正在跑的进程，同一批剩余工具只补桩不跑
- **分身只读** —— `strict_confirm` 只绑派发，工坊写入不再被误伤
- **Anthropic 循环** —— 卡死检测 + 流式中止，和 DeepSeek 那半对齐
- **压缩认前缀和小窗** —— 视觉 16K / 认不出窗户不再数 30 条就压；长会话免费修剪不再只写调用不给函数
- **grep / glob 超时** —— 不再退回 Python 整树硬扫把 daemon 卡死

> This is the release baseline after the kernel wave. 1.0.0 is reserved for after closed beta.
