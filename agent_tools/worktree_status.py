...
卷五十五 · P2 · 2026-06-03 · 让 Daemonkey 在对话里"看见"自己的 git 工作区真相。

档位：AUTO
  纯读 · 跑几条只读 git · 不动任何东西 · 任何场景都安全。

为什么这个工具至关重要：
  今天 (2026-06-03) Daemonkey 合主干时撞了一次车 —— Cursor 开的 master worktree 占着
  master 分支 · Daemonkey 一 merge 就被 git 拒。 根因是 Daemonkey **看不见**"现在工作区什么
  状态、master 是不是被别人占着、有没有未提交改动会被卷走"。

  这个工具把"工作区 / 跨 agent git 真相 + 该怎么处理"端到 Daemonkey 面前。 典型时机:
    - 准备 merge_wish_to_master / 切分支 / checkpoint 之前 → 先看一眼安不安全
    - BRO 说"你怎么又把自己弄崩了" → 先自检现场再动手
...
SPEC = ToolSpec(
    ...
    description=(
        ...
        "在并行改 / 是否占着 master)、遗留 stash、Daemonkey-last-good 回退点 · 外加大白话的"
